...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{GIFT Framework v3.1 --- Supplement S2}
\fancyhead[R]{\thepage}
...
\item The outputs match experiment to 0.087\% mean deviation
...
\begin{itemize}
\item Predictions are identical whether $T = 0$ (analytical) or $T = \kappa_T$ (capacity)
...
$T_{\text{analytical}}$ & Realized torsion for $\varphi = c \times \varphi_0$ & \textbf{0} (exact) \\
$T_{\text{physical}}$ & Effective torsion for interactions & \textbf{Open question} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Role in predictions}: $\kappa_T$ does NOT appear in any of the 18 dimensionless prediction formulas. It is a structural parameter characterizing $\Kseven$, not a physical observable.

\textbf{Compatibility}: $T_{\text{analytical}} = 0$ satisfies Joyce's bound ($\|T\| < 0.0288$) with infinite margin.
...
\textbf{Verification}: The analytical metric $g = (65/32)^{1/7} \times I_7$ has $\det(g) = [(65/32)^{1/7}]^7 = 65/32$ exactly, confirming the topological formula.
