...
\item The outputs match experiment to \textbf{0.24\%} mean deviation across 32 well-measured observables (PDG 2024 / NuFIT 6.0 / Planck 2020); the 33rd ($\delta_{\mathrm{CP}}$) lies at $1\sigma$ from NuFIT 6.0, raising the inclusive mean to 0.57\%
...
Quantity & Value \\
\midrule
Experimental (NuFIT 6.0, NO IC19) & $177^\circ \pm 20^\circ$ \\
GIFT prediction & $197^\circ$ (exact) \\
Deviation & 11.3\% (within $1\sigma$) \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Note}: NuFIT 6.0 (NO, IC19) reports $\delta_{\mathrm{CP}} = 177^\circ \pm 20^\circ$, placing the GIFT prediction at $1\sigma$. The earlier T2K+NOvA joint analysis (Nature, 2025) was consistent with ${\sim}197^\circ$ within uncertainties. The NuFIT 6.0 global fit notes consistency with CP conservation within $1\sigma$ for normal ordering, indicating this parameter is not yet well-constrained. DUNE (2028--2040) will resolve $\delta_{\mathrm{CP}}$ to $\pm 5^\circ$.
...
Quantity & Value \\
\midrule
Experimental (NuFIT 6.0) & $8.52^\circ \pm 0.11^\circ$ \\
...
\textit{Formula}:
$$\theta_{23} = \arctan\left(\sqrt{\frac{\dimE(\Gtwo)}{D_{\mathrm{bulk}}}}\right) = \arctan\left(\sqrt{\frac{14}{11}}\right) = 48.440^\circ$$

\textit{Components}:
\begin{itemize}
\item $\dimE(\Gtwo) = 14$: Holonomy group dimension
\item $D_{\mathrm{bulk}} = 11$: Bulk spacetime dimension ($= 4 + \dimE(\Kseven)$)
\item $\dimE(\Gtwo) + D_{\mathrm{bulk}} = 25 = \Weyl^2$: The identity $\Weyl^2 = \dimE(\Gtwo) + D_{\mathrm{bulk}}$
\end{itemize}

\textit{Physical interpretation}:
The atmospheric mixing angle $\theta_{23}$ governs $\tau$--$\mu$ flavor mixing. The tangent-squared $\tan^2\theta_{23} = \dimE(\Gtwo)/D_{\mathrm{bulk}}$ represents the ratio of internal holonomy to bulk spacetime degrees of freedom. This implies $\sin^2\theta_{23} = 14/25 = \dimE(\Gtwo)/\Weyl^2$, algebraically consistent with Relation \#14c.
...
Quantity & Value \\
\midrule
Experimental (NuFIT 6.0) & $48.5^\circ \pm 0.9^\circ$ \\
GIFT prediction & $48.440^\circ$ \\
Deviation & 0.12\% \\
\bottomrule
\end{tabular}
\end{table}
...
\textit{Formula}:
$$\theta_{12} = \arctan\left(\frac{\dimE(\Gtwo)}{b_2}\right) = \arctan\left(\frac{14}{21}\right) = \arctan\left(\frac{2}{3}\right) = 33.690^\circ$$

\textit{Components}:
\begin{itemize}
\item $\dimE(\Gtwo) = 14$: Holonomy group dimension
\item $b_2 = 21$: Second Betti number
\item $\dimE(\Gtwo)/b_2 = 2/3 = Q_{\mathrm{Koide}}$: The solar mixing tangent equals the Koide parameter
\end{itemize}

\textbf{Note}: This formula achieves an algebraic unification: $\tan(\theta_{12}) = 2/3$ implies $\sin^2\theta_{12} = 4/13$ exactly (Relation \#14b), resolving all internal consistency between angle and $\sin^2$ forms. The complexity cost drops from 52 (old $\gamma_{\text{GIFT}}$ formula) to 4 (two primary invariants, one division), and no unexplained integer coefficients remain.
...
Quantity & Value \\
\midrule
Experimental (NuFIT 6.0) & $33.68^\circ \pm 0.72^\circ$ \\
GIFT prediction & $33.690^\circ$ \\
Deviation & 0.03\% \\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection{Relation \#14c: $\sin^2\theta_{23}^{\mathrm{PMNS}} = 14/25$}

\textit{Formula}:
$$\sin^2\theta_{23}^{\mathrm{PMNS}} = \frac{\dimE(\Gtwo)}{\Weyl^2} = \frac{14}{25} = 0.56$$

\textit{Alternative expression}:
\begin{itemize}
\item $\dimE(\Gtwo)/(\dimE(\Gtwo) + D_{\mathrm{bulk}}) = 14/(14 + 11) = 14/25$
\end{itemize}

\textbf{Consistency check}: From Relation \#13, $\tan^2\theta_{23} = 14/11$, whence $\sin^2\theta_{23} = 14/(14+11) = 14/25$. The angle and $\sin^2$ forms are algebraically identical.
...
Quantity & Value \\
\midrule
Experimental (NuFIT 6.0) & $0.561 \pm 0.016$ \\
GIFT prediction & $0.56$ \\
Deviation & 0.18\% \\
\bottomrule
\end{tabular}
\end{table}
...
Quantity & Value \\
\midrule
Experimental (NuFIT 6.0) & $0.02195 \pm 0.0007$ \\
...
11 & $\delta_{\mathrm{CP}}$ & $\dimE_{\Kseven}{\times}\dimE_{\Gtwo}{+}H^*$ & $197^\circ$ & $177^\circ \pm 20^\circ$ & $1\sigma$ & \proven{} \\
12 & $\theta_{13}$ & $\pi/21$ & $8.57^\circ$ & $8.52^\circ$ & 0.60\% & \topomark{} \\
13 & $\theta_{23}$ & $\arctan(\sqrt{\dimE_{\Gtwo}/D_{\mathrm{bulk}}})$ & $48.44^\circ$ & $48.5^\circ$ & 0.12\% & \topomark{} \\
14 & $\theta_{12}$ & $\arctan(\dimE_{\Gtwo}/b_2)$ & $33.69^\circ$ & $33.68^\circ$ & 0.03\% & \topomark{} \\
...
\textbf{Mean deviation (32 well-measured observables)}: \textbf{0.24\%} (PDG 2024 / NuFIT 6.0 / Planck 2020)

\textbf{Mean deviation (all 33 incl.\ $\delta_{\mathrm{CP}}$)}: 0.57\%

$\delta_{\mathrm{CP}}$: $197^\circ$ vs $177^\circ \pm 20^\circ$ ($1.0\sigma$; experimental uncertainty $\pm 11\%$ --- see main paper \S9.1)

% ---

\section{Statistical Uniqueness of $(b_2=21, b_3=77)$}

The comprehensive Monte Carlo validation (3,070,396 configurations tested including 30 known $\Gtwo$ manifolds, zero outperforming GIFT) is presented in the main paper, Section 7. Key results: $> 4.2\sigma$ (three independent null models); Bayes factors 288--4{,}567 (decisive); Westfall--Young maxT 11/33 individually significant (global $p = 0.008$); Pareto-optimal. $\E_8\times\E_8$ achieves approximately $10\times$ better agreement than alternatives; $\Gtwo$ holonomy approximately $5\times$ better than Calabi--Yau.
...
12 & $\delta_{\mathrm{CP}}$ & $\dimE_{\Kseven}{\times}\dimE_{\Gtwo}{+}H^*$ & $197^\circ$ & $177^\circ$ & $1\sigma$ & 3 & DERIVED \\
13 & $\theta_{13}$ & $\pi/b_2$ & $8.57^\circ$ & $8.52^\circ$ & 0.60\% & 3 & DERIVED \\
14 & $\theta_{23}$ & $\arctan(\sqrt{\dimE_{\Gtwo}/D_{\mathrm{bulk}}})$ & $48.44^\circ$ & $48.5^\circ$ & 0.12\% & 2 & DERIVED \\
15 & $\theta_{12}$ & $\arctan(\dimE_{\Gtwo}/b_2)$ & $33.69^\circ$ & $33.68^\circ$ & 0.03\% & 2 & DERIVED \\
...
20 & $\sin^2\theta_{23}^{\mathrm{PMNS}}$ & $\dimE_{\Gtwo}/\Weyl^2$ & 14/25 & 0.561 & 0.18\% & 15 & ROBUST \\
21 & $\sin^2\theta_{13}^{\mathrm{PMNS}}$ & $D_{\mathrm{bulk}}/\dimE_{\E_8^2}$ & 11/496 & 0.02195 & 1.04\% & 5 & SUPPORTED \\
