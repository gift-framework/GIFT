...
\noindent\textbf{DOI}: \href{https://doi.org/10.5281/zenodo.19893371}{10.5281/zenodo.19893371}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{fig_weyl_spacing_paper_b.png}
\caption{Weyl exponent and level spacing for the unified KK tower. \emph{(a)} Convergence of the Weyl exponent $\alpha(\lambda_{\mathrm{cut}})$ as the cutoff is raised: stabilisation at $\alpha_{7D} = 3.460$ (vs.\ the theoretical $7/2 = 3.5$, deviation $1.1\%$); the convergence window is shaded. \emph{(b)} Histogram of the unfolded level spacing for the first 100 KK eigenvalues, compared to Poisson (integrable, $\chi^2 = 1.36$) and the GOE Wigner surmise ($\chi^2 = 28.0$); the data favour Poisson by more than an order of magnitude, consistent with the adiabatic product structure.}
\label{fig:weyl_spacing}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{fig_intersection_heatmap_paper_b.png}
\caption{K3 fiber pairing $I_{IJ}$ on the $b_2(M) = 21$ subspace. \emph{(a)} The $22 \times 22$ ambient K3 lattice pairing as a heatmap (diverging colormap, signature $(3, 19)$); the orange-bordered class is the lattice direction killed by TCS boundary matching, so it does not extend to a global harmonic 2-form on $M$. \emph{(b)} Eigenvalue spectrum (signed bars) of $I_{IJ}$ restricted to the 21 surviving global harmonic 2-forms: 3 self-dual (red, $\sim 5\text{--}8$) and 18 anti-self-dual (blue, $\sim -10^{-3}$), giving signature $(3, 18)$ and the SD/ASD spectral gap of $\sim 2{,}210$.}
...
\begin{itemize}
\item[{[A]}] de La Fourni\`ere, B. \emph{A Certified Torsion-Free $\Gtwo$ Structure on a TCS Neck Model via Computer-Assisted Proof}. Zenodo, 2026. DOI: \href{https://doi.org/10.5281/zenodo.19892350}{10.5281/zenodo.19892350}.
