...
\textbf{Multiplicity}: 33 independent predictions, not cherry-picked coincidences. Random matching at 0.26\% mean deviation across 33 quantities has probability $< 10^{-20}$ under a naive null model.
...
\textbf{Falsifiability}: DUNE will test $\delta_{\CP} = 197^\circ$ to $\pm 5^\circ$ precision by 2039. A single clear contradiction would strongly disfavor the framework.
...
\textbf{Experimental status}: The T2K+NOvA joint analysis (Nature, 2025)~\cite{t2knova2025} reports $\delta_{\CP}$ consistent with values in the range $\sim 180^\circ$--$220^\circ$ depending on mass ordering assumptions, with best-fit regions compatible with $197^\circ$ within uncertainties.
...
\midrule
$\theta_{12}$ & $\arctan(\sqrt{\delta/\gamma_{\mathrm{GIFT}}})$ & $33.40^\circ$ & $33.41 \pm 0.75^\circ$ & 0.03\% \\
$\theta_{13}$ & $\pi/b_2$ & $8.57^\circ$ & $8.54 \pm 0.12^\circ$ & 0.37\% \\
$\theta_{23}$ & $\arcsin((b_3 - p_2)/H^*)$ & $49.25^\circ$ & $49.3 \pm 1.0^\circ$ & 0.10\% \\
\bottomrule
\end{tabular}
\end{table}

The auxiliary parameters: $\delta = 2\pi/\Weyl^2 = 2\pi/25$ and $\gamma_{\mathrm{GIFT}} = (2 \times \rank(\E_8) + 5 \times H^*)/(10 \times \dimE(\Gtwo) + 3 \times \dimE(\E_8)) = 511/884$.
...
8 & $\delta_{\CP}$ & $7 \times 14 + 99$ & $197^\circ$ & $\sim 197^\circ$ & compat. & VERIFIED \\
9 & $\theta_{12}$ & $\arctan(\sqrt{\delta/\gamma})$ & $33.40^\circ$ & $33.41^\circ$ & 0.03\% & TOPOLOGICAL \\
10 & $\theta_{13}$ & $\pi/b_2$ & $8.57^\circ$ & $8.54^\circ$ & 0.37\% & TOPOLOGICAL \\
11 & $\theta_{23}$ & $\arcsin((b_3 - p_2)/H^*)$ & $49.25^\circ$ & $49.3^\circ$ & 0.10\% & TOPOLOGICAL \\
...
\begin{itemize}
\item Mean deviation: \textbf{0.26\%} (PDG 2024 / Planck 2020)
\item Median deviation: 0.10\%
\item Exact matches: 4 ($N_{\mathrm{gen}}$, $m_s/m_d$, $\delta_{\CP}$, $\Omega_{\mathrm{DM}}/\Omega_b$)
\item Sub-0.1\% deviation: 9
\item Sub-1\% deviation: 32 (97\%)
\item Maximum deviation: 1.13\% ($\sin^2\theta_{23}^{\mathrm{CKM}}$)
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../figures/fig5_predictions_light.png}
\caption{GIFT predictions vs experimental values on logarithmic scale. The 33 dimensionless predictions span six orders of magnitude with mean deviation 0.26\%.}
...
\textbf{Method}: Comprehensive Monte Carlo validation testing 192,349 alternative configurations:
\begin{itemize}
\item 100,000 random $(b_2, b_3)$ configurations
...
\item 91,896 full combinatorial configurations varying all parameters
...
\midrule
1 & $\mathbf{\E_8 \times \E_8}$ & 496 & \textbf{0.26\%} & \textbf{3.000} \\
...
The statistical significance ($p < 5 \times 10^{-6}$) applies to parameter variations. The formula-level analysis (Section 7.5) extends this to the space of formula structures within a defined grammar.
...
\item \textbf{Current data}: T2K+NOvA joint analysis consistent with $\sim 197^\circ$ within uncertainties~\cite{t2knova2025}
...
\item \textbf{Statistical analysis}: The configuration $(21, 77)$ is the unique optimum among 192,349 tested, not an arbitrary choice.
...
\begin{itemize}
\item \textbf{33 derived relations} with mean deviation 0.26\% (18 core + 15 extended)
\item \textbf{Formal verification} of arithmetic consistency (2400+ Lean 4 theorems, zero sorry, zero custom axioms)
\item \textbf{Statistical uniqueness} of $(b_2, b_3) = (21, 77)$ at $> 4.5\sigma$ among 192,349 alternatives
...
$\Weyl$ & Pentagonal index: $(\dimE(\Gtwo)+1)/N_{\mathrm{gen}}$ & 5 \\
