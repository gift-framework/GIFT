...
La version actuelle, GIFT v3.3, a franchi une étape supplémentaire. **Il n'y a plus de paramètre libre du tout.** La structure est entièrement fixée par le choix (*b₂*, *b₃*) = (21, 77) et les propriétés algébriques de E₈ × E₈. Trente-trois prédictions sans dimension se déduisent, avec une déviation moyenne de 0.24% sur les trente-deux observables bien mesurés. Dix-huit des relations algébriques centrales ont été vérifiées formellement en Lean 4.
...
**La phase de violation CP des neutrinos** : *δ*_CP = 7 × dim(G₂) + *H*\* = 7 × 14 + 99 = 197°. Une combinaison additive d'invariants topologiques. La mesure la plus récente (NuFIT 6.0) donne 177° ± 20°, compatible à 1σ. C'est la prédiction falsifiable la plus importante du cadre : l'expérience DUNE (2028–2040) mesurera cette phase avec une précision qui tranchera définitivement.
