...
theorem m_tau_m_e_exact : dim_K7 + 10 * dim_E8 + 10 * H_star = 3477 := by
  simp only [dim_K7, dim_E8, H_star, b2_K7, b3_K7]
  native_decide
...
/-- 10 × 248 = 2480 -/
theorem ten_times_E8 : 10 * dim_E8 = 2480 := by native_decide

/-- 10 × 99 = 990 -/
theorem ten_times_H_star : 10 * H_star = 990 := by
...
theorem m_tau_m_mu_structure : (b3_K7 - b2_K7 : ℚ) / N_gen = 56 / 3 := by
  simp only [b3_K7, b2_K7, N_gen]
  norm_num
...
theorem Q_Koide_exact : (dim_G2 : ℚ) / b2_K7 = 2 / 3 := by
  simp only [dim_G2, b2_K7]
  norm_num
...
theorem Q_Koide_from_N_gen : (2 : ℚ) / (N_gen : ℕ) = 2 / 3 := by
  simp only [N_gen]
  norm_num
...
theorem m_mu_m_e_structure : H_star / b2_K7 = 4 := by
  simp only [H_star, b2_K7, b3_K7]
  native_decide
...
theorem lepton_flavors : 2 * N_gen = 6 := by native_decide

/-- Leptons in matter decomposition: 12 (including antiparticles/chiralities) -/
theorem lepton_states : 4 * N_gen = 12 := by native_decide
