...
theorem delta_CP_formula : 7 * dim_G2 + H_star = 197 := by
  simp only [dim_G2, H_star, b2_K7, b3_K7]
  native_decide

/-- Intermediate: 7 × 14 = 98 -/
theorem seven_times_G2 : 7 * dim_G2 = 98 := by
  simp only [dim_G2]
  native_decide
...
/-- θ₂₃ numerator: 8 + 77 = 85 -/
theorem theta_23_numerator : 8 + b3_K7 = 85 := by
  simp only [b3_K7]
  native_decide

/-- sin²θ₂₃ = (8 + b₃)/H* = 85/99 -/
theorem theta_23_fraction : (8 + b3_K7 : ℚ) / H_star = 85 / 99 := by
  simp only [b3_K7, H_star, b2_K7]
  norm_num
...
theorem theta_12_fraction : (b2_K7 + 1 : ℚ) / (b3_K7 - b2_K7) = 11 / 28 := by
  simp only [b2_K7, b3_K7]
  norm_num

/-- 77 - 21 = 56 = 8 × 7 -/
theorem betti_difference : b3_K7 - b2_K7 = 56 := by native_decide
...
theorem tau_hierarchy :
  ...
  simp only [dim_E8xE8, b2_K7, dim_J3O, H_star, b3_K7]
  norm_num

/-- Intermediate: 496 × 21 = 10416 -/
theorem tau_numerator_full : dim_E8xE8 * b2_K7 = 10416 := by native_decide

/-- Intermediate: 27 × 99 = 2673 -/
theorem tau_denominator_full : dim_J3O * H_star = 2673 := by
  simp only [dim_J3O, H_star, b2_K7, b3_K7]
  native_decide
