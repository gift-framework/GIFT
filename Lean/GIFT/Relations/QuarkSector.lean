...
theorem ms_md_exact : 4 * Weyl_factor = 20 := by
  simp only [Weyl_factor]
  native_decide

/-- Alternative: m_s/m_d = b₂ - 1 = 21 - 1 = 20 -/
theorem ms_md_from_b2 : b2_K7 - 1 = 20 := by
  simp only [b2_K7]
  native_decide

/-- The two derivations agree -/
theorem ms_md_consistency : 4 * Weyl_factor = b2_K7 - 1 := by
  simp only [Weyl_factor, b2_K7]
  native_decide

/-! ## Bottom/Charm Ratio Structure -/

/-- m_b/m_c numerator from b₃ - b₂ = 56 -/
theorem mb_mc_numerator : b3_K7 - b2_K7 = 56 := by native_decide
...
theorem mt_mb_structure : dim_E8 / dim_G2 = 17 := by
  simp only [dim_E8, dim_G2]
  native_decide
...
theorem cabibbo_structure : (b2_K7 : ℚ) / b3_K7 = 21 / 77 := by
...
theorem quark_flavors : 2 * N_gen = 6 := by native_decide
